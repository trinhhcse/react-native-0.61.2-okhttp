...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    UIView *keyboardView = [[window subviews] firstObject];
    CGFloat keyboardHeight = [notification.userInfo[UIKeyboardFrameEndUserInfoKey] CGRectValue].size.height;
    //Remove DONE button
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *view = [keyboardView viewWithTag:10000];
        if (view) {
    ...
    if (self.keyboardType == UIKeyboardTypePhonePad ||
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self->doneButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 163, window.frame.size.width/3.f - 3, 53)];
            self->doneButton.tag = 10000;

            [self->doneButton setImage:[UIImage imageNamed:@"ic_hide_keyboard"]
                        forState:UIControlStateNormal];
            if (@available(iOS 11, *)) {
                [self->doneButton setImage:[UIImage imageNamed:@"ic_hide_keyboard_pressed"]
                            forState:UIControlStateHighlighted];
            }

            [self->doneButton.titleLabel setFont:[UIFont systemFontOfSize:18]];
            [self->doneButton addTarget:self action:@selector(reactBlur)
                 forControlEvents:UIControlEventTouchUpInside];

            [self->doneButton setTitleColor:[UIColor blackColor]
                             forState:UIControlStateNormal];


            CGFloat buttonHeight = window.frame.size.width/6;
            CGFloat buttonWidth = window.frame.size.width/3;
            if (window.frame.size.height >= 736.0) {
                buttonHeight = 55;
            }

            CGFloat buttonY = keyboardView.frame.size.height - buttonHeight;

            if (keyboardHeight/window.frame.size.width > 0.6667) {
                buttonY = keyboardView.frame.size.height - keyboardHeight + 3 * buttonHeight;
                if (window.frame.size.height < 736.0) {
                    buttonY = keyboardView.frame.size.height - buttonHeight;
                }
            }

            UIImage *imageDefault = [self imageWithColor:[UIColor colorWithRed:210/255.f
                                                                         green:213/255.f
                                                                          blue:219/255.f
                                                                         alpha:0.0f]
                                                    size:CGSizeMake(buttonWidth, buttonHeight)];

            UIImage *imageWhite = [self imageWithColor:[UIColor whiteColor]
                                                  size:CGSizeMake(buttonWidth, buttonHeight)];

            [self->doneButton setBackgroundImage:imageDefault forState:UIControlStateNormal];
            if (@available(iOS 11, *)) {
                [self->doneButton setBackgroundImage:imageDefault forState:UIControlStateHighlighted];
            }
            else {
                [self->doneButton setBackgroundImage:imageWhite forState:UIControlStateHighlighted];
            }
            ...
            [keyboardView addSubview:self->doneButton];
            [keyboardView bringSubviewToFront:self->doneButton];

            CGFloat duration = [[notification.userInfo objectForKey:UIKeyboardAnimationDurationUserInfoKey] floatValue] * 4/3.f -.02;
